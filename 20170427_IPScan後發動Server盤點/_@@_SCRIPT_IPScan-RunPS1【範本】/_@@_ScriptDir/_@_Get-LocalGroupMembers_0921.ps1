...
Begin {
	...
	$OutputFile = Join-Path $OutputDir _$LocalGroupName-LocalGroupMembers.csv
	Write-Verbose "Script will write the output to $OutputFile folder"
	Add-Content -Path $OutPutFile -Value "Scan-IP,LocalGroupName,Status,MemberType,MemberDomain,MemberName"
}
