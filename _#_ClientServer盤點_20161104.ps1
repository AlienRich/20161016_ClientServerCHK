...
$SubNet = Get-Content -Path $SubNetList | Foreach-Object { $SubNet = $_
	...
	$TitleInfo | out-file .\_$SubNet.xx_MachineModel.txt -append -encoding utf8

	$ServiceTitle = "Scan-IP,HOST_NAME,SERIAL_NO,Type,ServiceName,ServiceState"
	$ServiceTitle | out-file .\_$SubNet.xx_ServiceName.txt -append -encoding utf8

	##	雜訊過多，必要再使用	$PsinfoSoftwareTitle = "Scan-IP;Type;SoftwareName"
	##	雜訊過多，必要再使用	$PsinfoSoftwareTitle | out-file .\_$SubNet.xx_PsinfoSoftware.txt -append -encoding utf8
	$SoftwareTitle = "Scan-IP,HOST_NAME,SERIAL_NO,Type,SoftwareName"
	$SoftwareTitle | out-file .\_$SubNet.xx_Software.txt -append -encoding utf8

	$NetworkTitle = "Scan-IP,HOST_NAME,SERIAL_NO,Type,網卡IP,網卡資訊"
	$NetworkTitle | out-file .\_$SubNet.xx_NetWork.txt -append -encoding utf8
	...
	$PSinfoSYSTitle | out-file .\_$SubNet.xx_PSinfoSYSRpt.csv -append -encoding utf8
...
{$IP = "NoMsg"
 ...
 $ScanIP = "NoMsg"
 $ScanIP = "$IP"
 $host.ui.RawUI.WindowTitle = “USE-ID ::: $LoginID ::: Connection：$ScanIP ... ”
 
	if (test-connection $ScanIP -Quiet -Count 3) {
	 write-host "IP：$ScanIP,此IP存在。" -ForegroundColor GREEN
	 "IP：$ScanIP,此IP存在。" | Out-File -Append -FilePath ".\_$SubNet.xx_IP-No-response.log"
	 }
	 else
	 {
	 write-host "$ScanIP,NoResponse,無法Ping..." -ForegroundColor GREEN
	 "$ScanIP,NoResponse,無法Ping..." | Out-File -Append -FilePath ".\_$SubNet.xx_IP-No-response.log"
	 }
		...
		$PSinfoSYS = "NoMsg"
		$PSinfoSYS = .\psinfo.exe -d -c \\$ScanIP | out-file .\_$SubNet.xx_PSinfoSYSRpt.csv -append -encoding utf8
		...
		##	雜訊過多，必要再使用		$PsinfoSoftwareMSG | out-file .\_$SubNet.xx_PsinfoSoftware.txt -append  -encoding utf8
	 ...
	 if (-not $errorvar)
		...
		$ComputerSystemMsgALL | out-file .\_$SubNet.xx_MachineModel.txt -append -encoding utf8
	 ...
	 #################
		...
		$Diskinfo = (Get-WmiObject -Class Win32_LogicalDisk -computername $ScanIP -credential $CredMonitoradm |
			...
			$Diskinfo | Export-Csv -NoTypeInformation -Encoding UTF8 -Append -Path .\_$SubNet.xx_DiskInfo.csv
		...
		$ServiceItem = "NoMsg"
			foreach ($ServiceItem in $ServiceObject) 
		 		{
				$ServiceMSG = "NoMsg"
		 		$ServiceMSG = $ScanIP + "," + $ServerHostname + "," + $ServerSerialNumber + ",Service," + $ServiceItem.Name + "," + $ServiceItem.state
		 		$ServiceMSG | out-file ".\_$SubNet.xx_ServiceName.txt"  -append  -encoding utf8
		 		}
		...
		foreach ($GetSoftwareitem in $GetSoftware) 
		 	...
		 	if($softArr -Contains $GetSoftwareitem.ProgramName)
				...
				$GetSoftwareMsg = "NoMsg"
		 		...
		 		$GetSoftwareMsg | out-file .\_$SubNet.xx_Software.txt -append  -encoding utf8
		 		}
		 	}
		...
		foreach ($netitem in $netadapter)
			...
			$message | out-file ".\_$SubNet.xx_NetWork.txt"  -append -encoding utf8
			}
		...
		####----此部分以_@_Get-LocalGroupMembers_0921.ps1來撈-----
		.\_@_Get-LocalGroupMembers_0921.ps1 $ScanIP
		...
		"$ScanIP,RPC_Err,可連入C，但無法撈取WMI資料!!!" | Out-File -Append -FilePath ".\_$SubNet.xx_IP-No-response.log"
		$ScanIP = "NoMsg"
		}
	...
	"$ScanIP,NoPath,無此IP、非微軟系統。或帳號：$LoginID 無權限！C無法連入！！" | Out-File -Append -FilePath ".\_$SubNet.xx_IP-No-response.log"
	$ScanIP = "NoMsg"
	}
##NoTestPing }
##NoTestPing  else
...
##NoTestPing	"$ScanIP 無法Ping...HAHAHAHA~~" | Out-File -Append -FilePath ".\_$SubNet.xx_IP-No-response.log"
##NoTestPing  }
}
}
#####################################################################################################################
